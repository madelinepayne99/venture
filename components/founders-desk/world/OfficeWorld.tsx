...
type Props = {
  missions: readonly { state: string }[];
...
/** Drop-in client view. Pass the active workspace's real missions and existing handlers. */
export function OfficeWorld(props: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      engine.current = view; view.syncMissions(latest.current.missions, true); latest.current.onEngine?.(view);
  ...
  useEffect(() => { engine.current?.syncMissions(props.missions); }, [props.missions]);
  useEffect(() => { engine.current?.syncMissions(latest.current.missions, true); }, [props.workspaceId]);
